...
describe('starter Venore Rotworm pacing diagnostic', () => {
  it('measures the first kill through the production hunt flow across 64 seeds', () => {
    ...
    for (let seed = 1; seed <= 64; seed += 1) {
      ...
      let ticks = 0;
      const maxTicks = Math.ceil((3 * 60 * 1000) / TICK_MS);
    ...
    expect(killTimes).toHaveLength(64);
