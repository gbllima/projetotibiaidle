...
function applyServerSnapshot(session: HuntSession, server: CharacterView): void {
  ...
  session.character.stamina = server.stamina;
  ...
  session.character.supplies = server.supplies.map((stack) => ({ itemId: stack.itemId, count: stack.count }));
  session.character.warehouse = server.warehouse.map((stack) => ({ itemId: stack.itemId, count: stack.count }));
  session.character.backpackContents = (server.backpackContents ?? []).map((stack) => ({ itemId: stack.itemId, count: stack.count }));
  ...
  for (const stack of server.session?.loot ?? []) lootByItem[stack.itemId] = stack.count;
  ...
  if (server.live?.healCooldownTicks !== undefined) session.healCooldownTicks = server.live.healCooldownTicks;
}
