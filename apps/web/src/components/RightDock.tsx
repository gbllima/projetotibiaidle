...
export function RightDock({
  ...
  const [protectedItemIds, setProtectedItemIds] = useState<number[]>([]);
  const [autoSellEnabled, setAutoSellEnabled] = useState(false);
  const [autoSellPercent, setAutoSellPercent] = useState(90);
  ...
  useEffect(() => {
    ...
    void api.lootPreferences(character.id)
      .then((result) => {
        if (cancelled) return;
        const rich = result as typeof result & { protectedItemIds?: number[]; autoSell?: boolean; autoSellPercent?: number; sort?: boolean };
        setIgnoredItemIds(result.ignoredItemIds);
        setProtectedItemIds(rich.protectedItemIds ?? []);
        setAutoSellEnabled(Boolean(rich.autoSell));
        setAutoSellPercent(rich.autoSellPercent ?? 90);
        setRare(Boolean(rich.sort));
      })
      .catch(() => {
        if (cancelled) return;
        setIgnoredItemIds([]);
  ...
  const sellUnprotectedPouch = async () => {
    if (lootActionBusy || busy) return;
    const eligible = lootItems.filter((item) => item.count > 0 && !protectedItemIds.includes(item.itemId) && itemValue(item.itemId) > 0);
    ...
    try {
      ...
      setLootActionError(error instanceof Error ? error.message : 'Não foi possível vender o Loot Pouch.');
    } finally {
      setLootActionBusy(false);
    }
  };
  ...
  return <aside className="dock right">
    ...
    <DockBox id="loot-pouch" title="Loot Pouch" extra={<span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span>{lootItems.filter((s) => s.count > 0 && !ignoredItemIds.includes(s.itemId)).length} / {lootSlots}</span><button type="button" className="btn" disabled={busy || lootActionBusy} onClick={() => setLootConfigOpen(true)}>Config</button></span>}>
      ...
      <div className="box-tools">
        <button className="btn gold" disabled={busy || lootActionBusy || !lootItems.some((item) => item.count > 0 && !protectedItemIds.includes(item.itemId) && itemValue(item.itemId) > 0)} onClick={() => void sellUnprotectedPouch()}>{t('sell')}</button>
    ...
    {lootConfigOpen && <LootConfigModal
      character={view}
      lootItems={lootItems}
      onClose={() => setLootConfigOpen(false)}
      onChanged={(prefs, nextCharacter) => {
        setIgnoredItemIds(prefs.ignoredItemIds);
        setProtectedItemIds(prefs.protectedItemIds);
        setAutoSellEnabled(prefs.autoSell);
        setAutoSellPercent(prefs.autoSellPercent);
        setRare(prefs.sort);
        if (nextCharacter) {
          setView(nextCharacter);
          setPartyView(nextCharacter);
        }
      }}
    />}
