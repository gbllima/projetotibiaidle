...
import { ItemRarityBadge, visibleItemRarity } from './ItemRarityBadge.js';
...
export function ItemTooltip({
  ...
  return createPortal(
    <div className={`item-tooltip${rarity ? ` item-rarity-frame ${rarity}` : ''}`} style={{ left, top }} role="tooltip">
      <strong>{itemDisplayName(item)}</strong>
      {item.description && <p className="item-tooltip-desc">{item.description}</p>}
      <ItemRarityBadge itemId={itemId} />
